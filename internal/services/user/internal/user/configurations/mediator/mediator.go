...
import (
	...
	authCommondV1 "github.com/reoden/go-NFT/user/internal/user/features/checkauth/v1/commands"
	authDtosV1 "github.com/reoden/go-NFT/user/internal/user/features/checkauth/v1/dtos"
	creatingUserCommondV1 "github.com/reoden/go-NFT/user/internal/user/features/creatinguser/v1/commands"
...
func ConfigUserMediator(
	...
	err := mediatr.RegisterRequestHandler[*creatingUserCommondV1.CreateUser, *createUserDtosV1.CreateUserResponseDto](
		creatingUserCommondV1.NewCreateUserHandler(
			logger,
			userDBContext,
			userRepository,
			userOperateStreamRepository,
			cacheUserRepository,
			bloomFilter,
			tracer,
		),
	...
	err = mediatr.RegisterRequestHandler[*findUserByIdQueryV1.FindUserById, *findUserByIdDtosV1.FindUserByIdResponseDto](
		findUserByIdQueryV1.NewFindUserByIdHandler(
			logger,
			userDBContext,
			userRepository,
			cacheUserRepository,
	...
	err = mediatr.RegisterRequestHandler[*loginUserCommondV1.LoginUser, *loginUserDtosV1.LoginUserResponseDto](
		loginUserCommondV1.NewLoginUserHandler(
			logger,
			userDBContext,
			userRepository,
			userOperateStreamRepository,
	...
	err = mediatr.RegisterRequestHandler[*sendCaptchaCommondV1.SendCaptcha, *sendCaptchaDtosV1.SendCaptchaResponseDto](
		sendCaptchaCommondV1.NewSendCaptchaHandler(
			logger,
			userRepository,
			cacheUserRepository,
			tracer,
	...
	err = mediatr.RegisterRequestHandler[*logoutCommondV1.LogoutUser, *logoutDtosV1.LogoutUserResponseDto](
		logoutCommondV1.NewLogoutUserHandler(
			logger,
			userRepository,
			userOperateStreamRepository,
			cacheUserRepository,
	...
	err = mediatr.RegisterRequestHandler[*authCommondV1.AuthUser, *authDtosV1.AuthResponseDto](
		authCommondV1.NewAuthUserHandler(
			logger,
			userDBContext,
			userRepository,
			userOperateStreamRepository,
