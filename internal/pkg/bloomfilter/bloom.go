...
func NewRedisClient() *redis.Client {
	universalClient := redis.NewClient(&redis.Options{
		...
		DB:              0,  // use defaultLogger Database
...
func New(m uint, k uint, key string) *BloomFilter {
	return &BloomFilter{max(1, m), max(1, k), NewRedisClient(), key}
}

// From creates a new Bloom filter with len(_data_) * 64 bits and _k_ hashing
// functions. The data slice is not going to be reset.
func From(data []uint64, k uint, key string) *BloomFilter {
	m := uint(len(data) * 64)
	return FromWithM(data, m, k, key)
}

// FromWithM creates a new Bloom filter with _m_ length, _k_ hashing functions.
// The data slice is not going to be reset.
func FromWithM(data []uint64, m, k uint, key string) *BloomFilter {
	// Create a new BloomFilter with Redis backend
	bf := &BloomFilter{
		m:   m,
		k:   k,
		b:   NewRedisClient(),
		key: "bloom", // or you could generate a unique key
	}
...
// false positive rate
func NewWithEstimates(n uint, fp float64, key string) *BloomFilter {
...
func (f *BloomFilter) Add(data []byte) *BloomFilter {
	...
	for i := uint(0); i < f.k; i++ {
		f.b.SetBit(context.Background(), f.key, int64(f.location(h, i)), 1).Result()
	}
	return f
}
...
func (f *BloomFilter) Copy() *BloomFilter {
	fc := New(f.m, f.k, fmt.Sprintf("%s_%d", f.key, time.Now().UnixNano()))
	ctx := context.Background()
...
func (f *BloomFilter) Test(data []byte) bool {
	...
	for i := uint(0); i < f.k; i++ {
		...
		bit, _ := f.b.GetBit(context.Background(), f.key, int64(pos)).Result()
		if bit == 0 {
			return false
		}
	}
...
func EstimateFalsePositiveRate(m, k, n uint, key string) (fpRate float64) {
